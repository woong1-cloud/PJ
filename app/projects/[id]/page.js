...
export default function ProjectDetailPage() {
  ...
  async function toggleArchive() {
    const archiving = data.project.is_active;
    const message = archiving
      ? '이 프로젝트를 보관하시겠습니까?\n\n목록에서 숨겨지고 새 요구사항을 연결할 수 없게 됩니다.\n이미 연결된 요구사항은 그대로 남습니다.'
      : '이 프로젝트를 다시 활성화하시겠습니까?';
    ...
    await callApi(
      `/api/projects/${id}`,
      {
        ...
        body: JSON.stringify({ isActive: !archiving }),
      },
      archiving ? '보관 실패' : '보관 해제 실패',
    );
  }
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-400">
            ...
            {' · 기간 '}
            {data.project.start_date || data.project.target_date
              ? `${data.project.start_date ?? '미정'} ~ ${data.project.target_date ?? '미정'}`
              : '미정'}
          </p>
        </div>
        {globalAdmin && (
          <div className="flex flex-shrink-0 items-center gap-2">
            ...
            <button
              ...
              className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-50"
            >
              {data.project.is_active ? '보관' : '보관 해제'}
            </button>
          </div>
        )}
      </div>
