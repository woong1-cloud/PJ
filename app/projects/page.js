...
export default function ProjectsPage() {
  ...
  // 로드맵을 별도 주소로 빼지 않았다. 뷰만 바꾸는 것이라 브랜드 범위·보관 포함
  // 같은 조건을 그대로 이어서 봐야 하는데, 페이지를 넘기면 그 상태가 초기화된다.
  const [view, setView] = useState('list'); // 'list' | 'roadmap'
  // 보관된 프로젝트는 기본으로 숨긴다. 이 토글이 없으면 한 번 보관한 프로젝트를
  // 다시 찾아 해제할 방법이 없다(보관 해제 버튼이 상세 화면에 있으므로).
  ...
  useEffect(() => {
    ...
    // 목록 화면은 진척률 숫자만 쓴다. 로드맵일 때만 건별 제목·배포예상일을
    // 받아 온다 — 매번 실어 보내면 목록 화면이 쓰지도 않는 데이터를 나른다.
    if (view === 'roadmap') params.set('withRequirements', 'true');
  ...
  }, [scope, includeArchived, view, identity.brandId, reloadToken]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex gap-1">
          <ScopeButton active={view === 'list'} onClick={() => setView('list')}>
            목록
          </ScopeButton>
          <ScopeButton active={view === 'roadmap'} onClick={() => setView('roadmap')}>
            로드맵
          </ScopeButton>
        </div>
