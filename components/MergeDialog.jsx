...
export function MergeDialog({ source, onClose, onMerged }) {
  ...
  useEffect(() => {
    fetch(`/api/requirements/${source.id}/similar?brandId=${brandId}`)
      ...
      .catch(() => {});
    // includeDone 이 필요하다. 이미 완료된 요구사항도 병합 대상이 될 수 있다
    // ("이 요청은 지난달에 끝난 그 건과 같다"). 중복만 아래에서 걸러낸다.
    fetch(`/api/requirements?brandId=${brandId}&includeDone=true`)
      .then((res) => res.json())
      .then((d) => setAllReqs((d.requirements ?? []).filter((r) => r.id !== source.id && r.status !== MERGED_STATUS)))
      .catch(() => {});
  }, [source.id, brandId]);
