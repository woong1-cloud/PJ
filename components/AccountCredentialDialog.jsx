...
// props: open, onOpenChange, member(대상 팀원, {id, name, hasAccount}), onSaved()
// member.hasAccount 로 "계정 생성"/"비밀번호 재설정" 모드가 자동으로 정해진다.
// (예전에는 auth_user_id 를 봤지만, API가 내부 식별자 대신 불리언만 내려준다.)
export function AccountCredentialDialog({ open, onOpenChange, member, onSaved }) {
  const mode = member?.hasAccount ? 'reset' : 'create';
