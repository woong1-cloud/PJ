...
export function computeActionItems({ requirements, brands, roles } = {}) {
  ...
  const unassignedReview = reqs.filter(
    (r) => r.status === REVIEW_PENDING_STATUS && !r.assignee
  ).length;

  // 종결된 건에 예상일이 없는 것은 문제가 아니다 — 이미 끝났다.
  const noExpectedDate = reqs.filter(
    (r) => !CLOSED_STATUSES.includes(r.status) && !r.expected_release_date
  ).length;
