...
// 문지기로 인정하는 것들.
//
// myLaunchIds 는 목록 라우트(/api/launch GET)의 문지기다. 안에서
// getSessionMember 를 부르므로 로그인 안 한 사람은 거기서 막히고,
// 돌려주는 값이 곧 "내가 낀 런칭"이라 거르는 일까지 한다.
//
// 이 목록에 뭔가를 더할 때는 그것이 정말 "로그인 확인 + 범위 결정"을
// 둘 다 하는지 봐야 한다. 안 그러면 이 테스트가 통과만 하고 아무것도
// 지키지 않는다.
const GUARDS = ['requireGlobalAdmin', 'requireLaunchAccess', 'myLaunchIds'];
