...
import { REQUEST_RANGES, REQUEST_RANGE_LABELS } from './dateRange';
...
export function buildFilterFields({ teamMembers = [], categories = [], projects = [] } = {}) {
  return [
    ...
    // 맨 끝이다. 기획자가 자주 쓰는 축이지만 대부분의 사람에게는 아니고,
    // 앞쪽 일곱 개처럼 "이 값이면 이 건"으로 딱 갈리는 축도 아니다.
    {
      key: 'requestRange',
      label: '요청일',
      options: REQUEST_RANGES.map((value) => ({ value, label: REQUEST_RANGE_LABELS[value] })),
    },
  ];
}
