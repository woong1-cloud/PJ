...
describe('코드 형식', () => {
  ...
  //
  // parseRow 는 객체 행을 받는다(toObjects 가 만들어 준 모양) — 위 ROW 와
  // 같은 모양이 필요하다. cells() 는 배열 행(parseSheet 용)을 만들므로,
  // HEADER 와 짝지어 객체로 바꿔서 쓴다.
  const rowFrom = (over) => Object.fromEntries(HEADER.map((n, i) => [n, cells(over)[i]]));

  it('한 글자 접미가 붙은 코드를 받는다', () => {
    const row = rowFrom({ ID: '07B-02', '체크 항목': '재고 수정 권한 제한', 'D-day': -95 });
...
describe('제외 읽기', () => {
  ...
  it("진행상태가 '제외' 면 해당없음으로 들어온다", () => {
    const out = parseRow({
      sheetName: 'D03',
      row: { ...ROW, 진행상태: '제외', 비고: '[중복] 01-21에서 관리' },
    });
  ...
  it("상태가 '해당없음' 이어도 같다", () => {
    // 상태 열과 진행상태 열 둘 중 하나만 표시된 행이 실제로 둘 다 있다.
    const out = parseRow({ sheetName: 'D03', row: { ...ROW, 상태: '해당없음' } });
    expect(out.not_applicable).toBe(true);
  });

  it('비고가 비어 있으면 사유를 만들어 준다 — 사유 없는 해당없음은 없다', () => {
    const out = parseRow({ sheetName: 'D03', row: { ...ROW, 진행상태: '제외', 비고: '' } });
    expect(out.excluded_reason).toBe('양식에서 제외 표시됨');
  });

  it('보통 행은 해당없음이 아니다', () => {
    const out = parseRow({ sheetName: 'D01', row: ROW });
    expect(out.not_applicable).toBe(false);
    expect(out.excluded_reason).toBe(null);
  });

  it('그 밖의 상태 값은 여전히 안 읽는다', () => {
    // 시트의 진행 상태와 모아의 상태가 갈리면 어느 쪽이 맞는지 아무도 모른다.
    // '이번주' 같은 진행상태 값도 마찬가지다.
    const out = parseRow({ sheetName: 'D01', row: { ...ROW, 상태: '진행중', 진행상태: '이번주' } });
    expect(out.not_applicable).toBe(false);
