...
describe('isTaskSheet', () => {
  it('01~18 만 읽는다', () => {
    expect(isTaskSheet('01_신규법인')).toBe(true);
    expect(isTaskSheet('18_플랫폼구축')).toBe(true);
  });

  it('요약·사용법과 파생 시트는 건너뛴다', () => {
    // 19~24 는 목표역산·협조요청·진척률·간트·주간회의·R&R분배다. 파생을
    // 읽으면 모아가 계산할 것을 두 벌로 갖게 된다.
    for (const n of [
      '00_요약',
      '00_사용법',
      '19_목표역산',
      ...
      expect(isTaskSheet(n), n).toBe(false);
    }
  });

  it('이름이 없거나 모양이 다르면 거짓', () => {
    expect(isTaskSheet('Sheet1')).toBe(false);
    expect(isTaskSheet('')).toBe(false);
    expect(isTaskSheet(null)).toBe(false);
  });
});
...
describe('parseSheet · parseWorkbook', () => {
  ...
  it('파생 시트는 통째로 건너뛴다', () => {
    expect(parseSheet({ sheetName: '20_협조요청', rows: [HEADER, cells()] })).toEqual([]);
  });

  it('워크북에서 건너뛴 시트 이름을 돌려준다', () => {
    const out = parseWorkbook([
      { name: '01_신규법인', rows: [HEADER, cells()] },
      { name: '21_진척률', rows: [HEADER, cells()] },
      { name: '24_R&R분배', rows: [HEADER, cells()] },
    ]);
