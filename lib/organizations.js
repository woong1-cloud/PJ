...
//
// 조직이 20개쯤 되면 한 줄로 늘어놓을 수 없다. 브랜드와 본부는 고르는
...
// 스스로 권한을 부여하는 길이 된다.
//
// 모르는 값은 전부 가장 낮은 등급으로 떨어진다.
//
// TIER_RANK 가 아니라 BRAND_TIERS 로 본다. 1차는 브랜드별 등급이 아니라
// 저장할 수 없는 값인데, TIER_RANK 로 보면 '아는 값'이라 그대로 제안된다.
// 그러면 배치 창이 1차를 미리 채우고, 고를 목록에는 없고, 서버는 400 을
// 낸다 — 그 소속인 사람을 아예 배치할 수 없게 된다. 실제로 「사업부문」의
// 기본등급이 1차로 저장돼 있었다. hasOwnProperty 로 확인하는
...
export function suggestTierFromOrg(organization) {
  ...
  return BRAND_TIERS.includes(tier) ? tier : '4차';
}
