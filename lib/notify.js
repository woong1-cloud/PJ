...
import { loadLaunchMentionable } from './launchMentionable';
...
// 런칭 항목 댓글 알림.
//
// 후보 목록을 서버가 다시 만드는 것이 요점이다. 화면이 보내온 이름이 아니라
// loadLaunchMentionable 로 다시 만든 목록으로 본문을 해석하므로, 화면 목록을
// 조작해 남을 부르는 길이 없다 — lib/launchMentionable.js 의 "관문"이 여기서
// 완성된다.
export async function notifyLaunchComment({ launchId, taskId, actorId, body }) {
  try {
    if (!launchId || !taskId) return;
    const supabase = getSupabaseAdmin();

    // 항목을 다시 읽는다. 라우트가 들고 있는 행에는 id 밖에 없고(존재 확인용
    // select), 부르는 쪽에 컬럼을 맞추게 하면 언젠가 하나를 빠뜨려 그 자리만
    // 조용히 알림이 안 간다 — loadContext 가 요구사항 쪽에서 하는 판단과 같다.
    //
    // launch_id 를 함께 건다. 라우트가 이미 보지만, 여기서도 걸어 두면 다른
    // 곳에서 불러도 남의 런칭 항목을 읽지 않는다.
    const { data: task, error } = await supabase
      .from('launch_tasks')
      .select('id, code, title, assignee:team_members!launch_tasks_assignee_fkey(id, name)')
      .eq('id', taskId)
    ...
    // '@' 가 없으면 명단 조회를 건너뛴다. notifyComment 와 같은 최적화다 —
    // 댓글 대부분에는 멘션이 없고, 그때마다 참여자를 읽을 이유가 없다.
    let mentionedIds = [];
    if (typeof body === 'string' && body.includes('@')) {
      const mentionable = await loadLaunchMentionable(supabase, { launchId });
      mentionedIds = parseMentions(body, mentionable).map((m) => m.id);
    }

    const recipients = launchCommentRecipients(task, actorId, mentionedIds);
    // 받을 사람이 없으면 이름 조회까지 갈 필요가 없다. 담당자 없는 항목에
    // 혼잣말을 남기는 것이 흔한 쓰임이라 이 갈래가 실제로 자주 탄다.
    ...
    // 벨 문구에 코드를 붙인다. 런칭 항목 제목은 '상세페이지 오픈'처럼 짧고
    // 여러 런칭에 같은 것이 있어서, 코드가 없으면 어느 항목인지 안 읽힌다.
    const label = `${task.code} ${task.title}`;
    // 불려서 온 알림과 그냥 댓글 알림은 눌러야 할 이유가 다르다. 수신자는
    // 이미 합쳐져 있으므로 한 사람에게 두 줄이 가지 않고, 문구만 갈라 붙인다.
    const forComment = commentMessage(actorName, label);
    const forMention = mentionMessage(actorName, label);

    // link 에 항목 주소를 넣는다. requirement_id 는 넣지 않는다 — 런칭 항목은
    // 요구사항이 아니고, 그 컬럼은 requirements 를 가리키는 FK 라 넣을 값도 없다.
    // 그래서 이 알림은 link 가 없으면 눌러도 갈 곳이 없다(notificationHref).
    const link = `/launch/${launchId}?task=${encodeURIComponent(task.code)}`;

    await runQuietly('런칭 댓글 인앱', () =>
      insertNotificationRows(
        supabase,
        recipients.map((r) => ({
          team_member_id: r.id,
    ...
    // 메일은 불린 사람에게만. 일반 댓글까지 메일로 내보내면 활발한 항목 하나가
    // 하루에 열 통을 만든다 — 이름이 불린 것은 "당신 답을 기다린다"는 뜻이라
    // 성격이 다르다.
    const mentionedRecipients = recipients.filter((r) => r.mentioned).map((r) => r.id);
    if (mentionedRecipients.length > 0) {
      await runQuietly('런칭 멘션 메일', async () => {
        const emails = await loadEmails(supabase, mentionedRecipients);
        await sendMailToMany(
          mentionedRecipients.map((id) => emails.get(id)),
          launchMentionEmail({
    ...
    logFailure('런칭 댓글', error);
  }
}

// 협조 요청 알림 — 고른 역할의 참여자에게 벨과 메일 둘 다.
//
// 요청의 본체는 댓글이고(마이그레이션 0037) 그건 이미 저장됐다. 그래서 여기서
// 나는 오류는 전부 삼킨다 — 메일이 실패했다고 화면에 '요청 실패'가 뜨면 그건
// 거짓말이고, 사람은 되지도 않는 재시도를 하며 댓글만 두 번 남긴다.
//
// 받는 사람을 여기서 다시 만들지 않는다. notifyLaunchComment 가 후보 목록을
// 서버에서 다시 만드는 것과 다른 점인데, 저쪽은 사용자가 친 본문을 해석해야
// 해서 관문이 필요하고 이쪽은 라우트의 recipientsForRoles 가 이미 그 관문이다
// (0명이면 400 이라 요청 자체가 성립하지 않는다).
export async function notifyHelpRequest({
  ...
  try {
    if (!launchId || tasks.length === 0) return;
    // 보낸 사람 자신은 뺀다. 요청한 역할에 자기가 들어 있는 것이 흔하다
    // (개발이 개발에 묻는 일은 없지만 결정권을 겸한 사람이 있다) — 자기가
    // 보낸 메일을 자기가 받으면 그때부터 이 메일은 소음이 된다.
    const ids = [...new Set((recipientIds ?? []).filter(Boolean))].filter((id) => id !== actorId);
    if (ids.length === 0) return;

    const supabase = getSupabaseAdmin();

    // 라우트가 이름을 안 들고 있어서 여기서 읽는다. notifyLaunchComment 가
    // actorName 을 자기 안에서 읽는 것과 같은 자리다 — 부르는 쪽에 컬럼을
    // 맞추게 하면 언젠가 하나를 빠뜨리고, 그 자리만 조용히 이름이 빈다.
    const { data: actor, error: actorError } = await supabase
      .from('team_members')
      .select('name')
      .eq('id', actorId)
    ...
    // 런칭 이름은 메일 제목에만 쓴다. 없으면 제목에서 빠질 뿐이라 조회가
    // 실패해도 알림은 나간다 — notifySubmitted 의 브랜드 이름과 같다.
    ...
    const who = actorName ? `${actorName}님이` : '누군가';
    const what = roles.filter(Boolean).join(' · ');
    const label = one ? `'${one.code} ${truncateTitle(one.title)}'` : `런칭 항목 ${tasks.length}건`;

    // link 에 항목 주소를 넣는다. requirement_id 는 넣지 않는다 — 그 컬럼은
    // requirements 를 가리키는 FK 라 런칭 항목 id 를 넣을 수 없다
    // (notifyLaunchComment 와 같은 이유).
    const link = one
      ? `/launch/${launchId}?task=${encodeURIComponent(one.code)}`
      : `/launch/${launchId}`;

    await runQuietly('협조 요청 인앱', () =>
      insertNotificationRows(
        supabase,
        ids.map((id) => ({
          team_member_id: id,
          message: `${who} ${what}에 협조를 요청했습니다 — ${label}`,
          link,
        }))
      )
    );

    // 메일도 보낸다. 협조 요청은 "지금 당신이 답을 줘야 그 건이 움직인다"이고,
    // 벨은 로그인해야 보인다 — 막힌 항목을 며칠 더 세워 두는 것이 정확히
    // 이 기능이 없애려는 일이다.
    await runQuietly('협조 요청 메일', async () => {
      const emails = await loadEmails(supabase, ids);
      if (emails.size === 0) return;
      await sendMailToMany(
        ids.map((id) => emails.get(id)),
        helpRequestEmail({
          tasks,
          roles,
          actorName,
...
// "당신이 처리해야 할 일이 생겼다"이다. 배치는 /settings/members 에서만 하고 그
