import { DONE_STATUS, NA_STATUS, TODO_STATUS } from './launchTask';

// 재가져오기 계획.
//
// 같은 파일을 열 번 올려도 안전해야 한다. 그러지 않으면 아무도 두 번 안
// 올리고, 그러면 양식을 만든 의미가 없다.
//
// 규칙 하나가 나머지를 정한다: 계획은 엑셀이 원본, 진행은 모아가 원본.
// 재가져오기가 지난주 회의에서 찍은 상태를 지우면 그 순간 못 쓰는 기능이
// 된다.

// 가져오기가 뺀 것의 사유. 사람이 쓴 사유와 가르는 표식이다.
//
// 컬럼을 따로 두지 않는 이유: 사유가 어차피 필수다. 필수인 값 하나로
// 구분되면 컬럼을 더 두는 것은 같은 사실을 두 벌 갖는 것이다.
export const IMPORT_EXCLUDED_REASON = '양식에서 빠짐';

// 서버는 사유 뒤에 직전 상태를 붙여 저장한다 —
//   '양식에서 빠짐 (직전 상태: 하는 중)'
//
// 되살릴 때 '할 것' 으로만 돌아가면 "하는 중이었는데"가 사라진다. 컬럼을
// 하나 더 두는 대신 사유 문자열에 담는다. 사유는 어차피 사람이 읽는 것이고,
// 거기 적혀 있는 것이 가장 정직하다.
const PREV = /\(직전 상태:\s*([^)]+)\)/;

function importExcluded(row) {
  return String(row?.excluded_reason ?? '').startsWith(IMPORT_EXCLUDED_REASON);
}

function previousStatus(row) {
  const m = String(row?.excluded_reason ?? '').match(PREV);
  return m ? m[1].trim() : TODO_STATUS;
}

// 엑셀이 원본인 열. 이 목록에 없는 것은 절대 안 덮는다.
const PLAN_COLUMNS = [
  'title', 'workstream', 'category', 'channel', 'decision_org', 'owner_org',
  'owner_role', 'support_role', 'depends_on', 'day_offset', 'deliverable',
  'note', 'is_critical', 'sort_order', 'plain_text',
];

function planPatch(item) {
  const patch = {};
  for (const key of PLAN_COLUMNS) {
    if (item[key] !== undefined) patch[key] = item[key];
  }
  return patch;
}

// 값이 같은가.
//
// 빈 값의 모양이 여럿이다 — null · undefined · ''. 그냥 !== 로 비교하면
// 안 바뀐 줄이 "바뀜"으로 잡히고, 476건이 전부 바뀐 것처럼 보인다.
//
// depends_on 은 배열이라 자리까지 같아야 같다. 선행조건의 순서는 뜻이
// 없지만, 순서를 무시하려고 정렬해 비교하면 시트에서 순서를 고친 것이
// "안 바뀜"이 된다 — 사람이 고친 것은 보여주는 쪽이 맞다.
function same(a, b) {
  if (Array.isArray(a) || Array.isArray(b)) {
    const x = Array.isArray(a) ? a : [];
    const y = Array.isArray(b) ? b : [];
    return x.length === y.length && x.every((v, i) => String(v) === String(y[i]));
  }
  const norm = (v) => (v === null || v === undefined || v === '' ? null : v);
  const x = norm(a);
  const y = norm(b);
  if (x === null || y === null) return x === y;
  // 숫자와 숫자 문자열을 같게 본다. DB 에서 온 값과 파서가 만든 값의
  // 타입이 다를 수 있다.
  if (typeof x === 'number' || typeof y === 'number') return Number(x) === Number(y);
  if (typeof x === 'boolean' || typeof y === 'boolean') return Boolean(x) === Boolean(y);
  return String(x) === String(y);
}

// 화면에 보여줄 열 이름. 시트의 열 이름과 맞춘다 — 사람이 엑셀을 열어
// 대조할 것이기 때문이다.
export const PLAN_LABELS = {
  title: '체크 항목',
  workstream: '워크스트림',
  category: '대분류',
  channel: '채널',
  decision_org: '결정권',
  owner_org: '소속',
  owner_role: '주관',
  support_role: '지원',
  depends_on: '선행조건',
  day_offset: 'D-day',
  deliverable: '산출물',
  note: '비고',
  plain_text: '쉬운 설명',
  is_critical: '핵심(★)',
};

export function planReimport({ incoming = [], existing = [] } = {}) {
  const rows = incoming ?? [];
  const have = new Map((existing ?? []).map((e) => [e.code, e]));
  const seen = new Set(rows.map((i) => i.code));

  const create = [];
  const update = [];
  const restore = [];
  const markNa = [];
  const busy = [];
  const unchanged = [];
  const untouched = [];

  for (const item of rows) {
    const cur = have.get(item.code);

    if (!cur) {
      create.push({
        ...planPatch(item),
        code: item.code,
        source: 'import',
        status: item.not_applicable ? NA_STATUS : TODO_STATUS,
        excluded_reason: item.not_applicable ? item.excluded_reason : null,
      });
      continue;
    }

    // 손으로 고친 줄은 엑셀에서 독립한다. 계획 열도 안 덮는다 —
    // 안 그러면 회의 중에 고친 문구가 다음 업로드에 조용히 날아간다.
    if (cur.source === 'manual') {
      untouched.push({ id: cur.id, code: cur.code, why: 'manual' });
      continue;
    }

    // 실제로 다른 열만 모은다.
    //
    // 전부 update 에 넣으면 미리보기가 "갱신 476건"이라고 쓴다. 3건만
    // 바뀌었어도. 그러면 확인 버튼을 누르는 것이 검토가 아니라 도박이 되고,
    // 서버는 476번 UPDATE 를 날린다.
    const patch = planPatch(item);
    const changes = [];
    for (const [key, next] of Object.entries(patch)) {
      if (same(cur[key], next)) continue;
      changes.push({ field: key, label: PLAN_LABELS[key] ?? key, from: cur[key] ?? null, to: next });
    }
    if (changes.length > 0) {
      update.push({ id: cur.id, code: cur.code, title: cur.title ?? item.title, patch, changes });
    } else {
      unchanged.push({ id: cur.id, code: cur.code });
    }

    // 양식이 "이건 안 함"이라고 말한다.
    //
    // 브랜드가 양식을 채우는 흐름의 핵심이 여기다. 1차 업로드로 451건이
    // 들어간 뒤 브랜드가 "앱 18건은 안 함"을 표시해 다시 올리는데, 그 18건은
    // 이미 있는 항목이라 계획 열만 덮으면 해당없음이 통째로 무시된다.
    //
    // 상태를 안 건드린다는 원칙의 예외인 이유: '해당없음'은 진행 상태가
    // 아니라 범위다. "이 브랜드에는 안 하는 일"이고, 그 판단은 엑셀이 원본이다.
    if (item.not_applicable) {
      if (cur.status === TODO_STATUS) {
        markNa.push({ id: cur.id, code: cur.code, excluded_reason: item.excluded_reason });
      } else if (cur.status !== NA_STATUS) {
        // 하는 중·막힘·완료. 사람이 이미 붙어 있는 일을 파일이 지우면 안 된다 —
        // 양식이 늦게 반영된 것일 수 있다. 알리기만 하고 사람이 정한다.
        busy.push({ id: cur.id, code: cur.code, status: cur.status });
      }
      // 이미 해당없음이면 아무 일도 안 한다. 사유는 먼저 쓴 것이 남는다.
      continue;
    }

    // 가져오기가 뺐던 것이 다시 나타났다. 사람이 뺀 것은 안 건드린다 —
    // 사람의 판단이 파일보다 세다.
    //
    // startsWith 인 것이 요점이다. 서버가 직전 상태를 뒤에 붙여 저장하므로
    // 완전 일치로 검사하면 되살리기가 조용히 안 된다.
    if (cur.status === NA_STATUS && importExcluded(cur)) {
      restore.push({ id: cur.id, code: cur.code, status: previousStatus(cur) });
    }
  }

  const exclude = [];

  for (const cur of existing ?? []) {
    if (seen.has(cur.code)) continue;

    // 이미 한 일을 없던 일로 만들 수 없다.
    if (cur.status === DONE_STATUS) {
      untouched.push({ id: cur.id, code: cur.code, why: 'done' });
      continue;
    }
    if (cur.status === NA_STATUS) {
      untouched.push({ id: cur.id, code: cur.code, why: 'already' });
      continue;
    }
    // 시트 밖에서 난 일이라 시트에 없는 게 당연하다.
    if (cur.source === 'manual') {
      untouched.push({ id: cur.id, code: cur.code, why: 'manual' });
      continue;
    }

    exclude.push({
      id: cur.id,
      code: cur.code,
      // 뺄 때의 상태를 사유에 함께 적는다. 되살릴 때 '할 것' 으로만
      // 돌아가면 "하는 중이었는데" 가 사라진다.
      excluded_reason: IMPORT_EXCLUDED_REASON,
      previous_status: cur.status,
    });
  }

  return { create, update, exclude, restore, markNa, busy, unchanged, untouched };
}
