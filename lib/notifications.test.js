...
describe('submitRecipients', () => {
  const roles = [
    { team_member_id: 'a2', tier: '2차' },
    { team_member_id: 'b3', tier: '3차' },
    { team_member_id: 'c4', tier: '4차' },
    { team_member_id: 'd1', tier: '1차' },
  ];

  it('3차 이상만 받는다 — 4차 요청자는 빼고', () => {
    expect(submitRecipients(roles, 'someone')).toEqual(['a2', 'b3', 'd1']);
  });

  it('행위자 본인은 빼고 준다', () => {
    // 실무자도 자기 요구사항을 올릴 수 있다. 그때 자기 벨이 울리면 안 된다.
    expect(submitRecipients(roles, 'b3')).toEqual(['a2', 'd1']);
  });

  it('actorId 를 몰라도 3차 이상에게는 보낸다', () => {
    // resolveRecipients 와 갈리는 지점이다. 저쪽은 본인 제외를 못 지키면
    // 아예 안 보내지만(요청자·담당자 둘뿐이라 본인일 확률이 높다), 이쪽은
    // 수신자가 등급으로 정해져서 본인이 섞여도 나머지에게 가는 편이 낫다.
    expect(submitRecipients(roles, null)).toEqual(['a2', 'b3', 'd1']);
  });

  it('같은 사람이 두 줄이어도 하나만 받는다', () => {
    expect(
      submitRecipients(
        [
          { team_member_id: 'x', tier: '3차' },
          { team_member_id: 'x', tier: '2차' },
        ],
        'actor'
      )
    ).toEqual(['x']);
  ...
  it('빈 입력·모르는 등급에 터지지 않는다', () => {
    expect(submitRecipients()).toEqual([]);
    expect(submitRecipients(null, 'a')).toEqual([]);
    expect(submitRecipients([{ team_member_id: 'x', tier: '없는등급' }], 'a')).toEqual([]);
    expect(submitRecipients([{ tier: '2차' }], 'a')).toEqual([]);
  });

  it('3차 이상이 아무도 없으면 빈 배열 — 조용히 아무 일도 안 일어난다', () => {
    expect(submitRecipients([{ team_member_id: 'c4', tier: '4차' }], 'actor')).toEqual([]);
  });
});

describe('submitMessage', () => {
  it('할 일로 끝난다 — 읽고 넘길 알림이 아니다', () => {
    expect(submitMessage('김요청', '타임세일 개선')).toBe(
      "김요청님이 '타임세일 개선' 검토를 요청했습니다 — 담당자 지정이 필요합니다"
    );
  });

  it('이름을 못 찾아도 "님이" 로 시작하지 않는다', () => {
    expect(submitMessage(null, '제목')).toBe(
      "누군가 '제목' 검토를 요청했습니다 — 담당자 지정이 필요합니다"
    );
  });
});
...
describe('메시지 문구', () => {
  ...
  it('사유를 주면 뒤에 붙는다', () => {
    expect(statusMessage('김관리', '결제 버튼', '반려', '중복 요청입니다')).toBe(
      "김관리님이 '결제 버튼' 상태를 '반려'로 바꿨습니다 — 중복 요청입니다"
    );
  });

  it('사유가 길면 자른다 — 벨은 한 줄이라 앞이 밀려 나가면 안 된다', () => {
    const long = '가'.repeat(60);
    const got = statusMessage('김관리', '결제 버튼', '반려', long);
    expect(got).toContain('…');
    expect(got.endsWith('…')).toBe(true);
  });

  it('빈 사유는 붙이지 않는다', () => {
    expect(statusMessage('김관리', '결제 버튼', '반려', '   ')).toBe(
      "김관리님이 '결제 버튼' 상태를 '반려'로 바꿨습니다"
    );
  });
...
describe('signupMessage', () => {
  it('조직 이름을 그대로 넣는다', () => {
    // 예전에는 '스파오(브랜드)' 처럼 소속과 브랜드를 조합했다. 소속이 실제
    // 조직명이 되면서 두 값이 하나로 합쳐졌다.
    expect(signupMessage('김지원', '스파오')).toBe(
      '김지원님이 스파오 소속으로 가입했습니다 — 배치가 필요합니다'
    );
  });

  it('브랜드가 아닌 조직도 같은 문장이다', () => {
    expect(signupMessage('김지원', '법무팀')).toBe(
      '김지원님이 법무팀 소속으로 가입했습니다 — 배치가 필요합니다'
    );
  });

  it('이름을 모르면 "님이"로 시작하지 않는다', () => {
    expect(signupMessage(null, '법무팀')).toBe(
      '새 사용자가 법무팀 소속으로 가입했습니다 — 배치가 필요합니다'
    );
  });

  it('무엇을 해야 하는지로 끝난다', () => {
    expect(signupMessage('김지원', '본부', null)).toContain('배치가 필요합니다');
  });
});

describe('notificationHref', () => {
  it('link 가 있으면 link 로 간다', () => {
    expect(notificationHref({ link: '/admin/members', requirement_id: null })).toBe(
      '/admin/members'
    );
  });

  it('link 가 우선이다 — 둘 다 있어도 link', () => {
    expect(notificationHref({ link: '/admin/members', requirement_id: 'r1' })).toBe(
      '/admin/members'
    );
  });

  it('link 가 없던 예전 알림은 요구사항으로 간다', () => {
    expect(notificationHref({ requirement_id: 'r1' })).toBe('/requirements/r1');
  });

  it('갈 곳이 없으면 null 이다', () => {
    expect(notificationHref({})).toBe(null);
    expect(notificationHref(null)).toBe(null);
  });
});

describe('shouldRetryWithoutLink', () => {
  const rows = [{ team_member_id: 'm1', message: '가입', link: '/admin/members' }];

  it('INSERT 가 내는 PGRST204 를 잡는다', () => {
    // 이걸 놓쳐서 가입 알림이 인앱도 메일도 못 나갔다. PostgREST 는 INSERT 에서
    // 없는 컬럼을 만나면 42703 이 아니라 이 코드를 준다.
    expect(shouldRetryWithoutLink({ code: 'PGRST204' }, rows)).toBe(true);
  });

  it('SELECT 가 내는 42703 도 잡는다', () => {
    expect(shouldRetryWithoutLink({ code: '42703' }, rows)).toBe(true);
  });

  it('link 를 안 넣은 행이면 재시도하지 않는다 — 재시도해도 결과가 같다', () => {
    const noLink = [{ team_member_id: 'm1', message: '코멘트' }];
    expect(shouldRetryWithoutLink({ code: 'PGRST204' }, noLink)).toBe(false);
  });

  it('다른 오류는 삼키지 않는다', () => {
    // 여기서 참을 주면 진짜 실패가 "link 때문"으로 위장되고, 재시도까지 실패해
    // 원인이 두 겹으로 숨는다.
    expect(shouldRetryWithoutLink({ code: '23503' }, rows)).toBe(false);
    expect(shouldRetryWithoutLink({ code: '23505' }, rows)).toBe(false);
  });

  it('오류가 없으면 거짓', () => {
    expect(shouldRetryWithoutLink(null, rows)).toBe(false);
    expect(shouldRetryWithoutLink(undefined, rows)).toBe(false);
  });

  it('행을 안 넘겨도 터지지 않는다', () => {
    expect(() => shouldRetryWithoutLink({ code: 'PGRST204' })).not.toThrow();
    expect(shouldRetryWithoutLink({ code: 'PGRST204' })).toBe(false);
  });
});
...
describe('launchCommentRecipients', () => {
  ...
  it('actorId 가 없으면 아무에게도 안 간다', () => {
    expect(launchCommentRecipients(task({ assignee: { id: 'a' } }), null, ['b'])).toEqual([]);
  });
});
