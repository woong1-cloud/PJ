...
describe('signupMessage', () => {
  it('브랜드 소속이면 어느 브랜드인지 넣는다', () => {
    expect(signupMessage('김지원', '브랜드', '스파오')).toBe(
      '김지원님이 스파오(브랜드) 소속으로 가입했습니다 — 배치가 필요합니다'
    );
  });

  it('본부 소속이면 브랜드 없이 소속만 넣는다', () => {
    expect(signupMessage('김지원', '본부', null)).toBe(
      '김지원님이 본부 소속으로 가입했습니다 — 배치가 필요합니다'
    );
  });

  it('이름을 모르면 "님이"로 시작하지 않는다', () => {
    expect(signupMessage(null, '본부', null)).toBe(
      '새 사용자가 본부 소속으로 가입했습니다 — 배치가 필요합니다'
    );
  });
