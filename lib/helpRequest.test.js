...
describe('recipientsForRoles', () => {
  ...
  // 이걸 안 하면 라우트가 돌려주는 sent 가 실제 발송보다 1 크다.
  // notifyHelpRequest 는 자기를 빼고 보내는데 라우트는 빼기 전 수를 세고,
  // 화면은 그 수를 그대로 「N명에게 보냈습니다」로 말한다.
  it('보내는 사람 자신은 뺀다', () => {
    expect(recipientsForRoles(members, ['물류팀'], 'a').map((m) => m.id)).toEqual(['c']);
  });

  it('그 역할이 나 혼자면 빈 목록 — 부르는 쪽이 이유를 갈라 봐야 한다', () => {
    expect(recipientsForRoles(members, ['CAIO실'], 'b')).toEqual([]);
  });

  it('excludeId 가 없으면 아무도 안 뺀다 — 기존 부르는 자리가 그대로 돈다', () => {
    expect(recipientsForRoles(members, ['물류팀'], null).map((m) => m.id).sort())
      .toEqual(['a', 'c']);
  });
});
