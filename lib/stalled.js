...
// "모른다"가 아니다.
//
// awaitingAnswer 는 요청자에게 물어보고 답을 기다리는 중이라는 뜻이다. 그때는
// 정체가 아니다 — 공이 저쪽에 있는데 '멈춘 것'으로 세면 우리가 안 집은 것처럼
// 보인다. 실제로 검토대기 중앙값 17일 중 일부가 이 경우였다.
//
// stalledDays 는 그대로 잰다(여기서 null 로 만들지 않는다). 답을 60일 안 주는
// 것도 언젠가 드러나야 하고, 화면은 '3일째 답 기다림'을 그려야 한다.
export function isStalled({ status, stalledDays, awaitingAnswer } = {}) {
