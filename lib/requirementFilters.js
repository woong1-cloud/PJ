...
export const FILTER_KEYS = [
  'status',
  // 유형(신규/개선/오류/문의). "오류만 보기"가 실제로 자주 필요한 축이라
  // 필터바에 칸을 준다 — missing 처럼 링크 전용으로 두면 아무도 못 쓴다.
  'type',
  'assignee',
  'category',
  'channel',
  'priority',
  'project',
  // 요청일 범위(thisWeek | thisMonth | lastMonth | last3Months).
  // 값이 날짜가 아니라 키인 것이 요점이다 — 날짜 두 칸을 주면 필터바가
  // 무거워지고, 47건 규모에서 "9월 1일~15일"을 정확히 짚을 일이 드물다.
  'requestRange',
];
...
export function parseFilterParams(searchParams) {
  ...
  return {
    ...
    mine: get('mine') === 'true',
    // 대시보드 '손볼 것'이 링크로만 넣는 값(assignee | expectedDate).
    // FILTER_KEYS 에 넣지 않는다 — 필터바에 셀렉트를 하나 더 만들면 평소에
    // 아무도 안 쓰는 칸이 늘어난다. 여기로 들어온 사람에게는 아래 안내가
    // 대신 보인다(MissingNotice).
    missing: get('missing'),
    // 빠른 필터 '지연'. FILTER_KEYS 에 넣지 않는 이유는 missing 과 같다 —
    // 셀렉트로 고르는 값이 아니라 칩이 켜고 끄는 값이다.
    overdue: get('overdue') === 'true',
    // 빠른 필터 '멈춘 것'. overdue 와 같은 이유로 FILTER_KEYS 밖이다.
    // 판정이 change_logs·코멘트를 봐야 나오므로 서버가 계산해서 거른다.
    stalled: get('stalled') === 'true',
  };
}
...
export function buildRequirementsQuery({
  ...
  // FILTER_KEYS 밖이라 따로 실어 준다. 아는 두 값만 통과시킨다 — 주소를 손으로
  // 고쳐 아무 컬럼 이름이나 넣는 길을 열어 두지 않는다.
  if (missing === 'assignee' || missing === 'expectedDate' || missing === 'redmine') {
...
// 모바일 필터 시트 버튼에 붙는 숫자.
//
// hasActiveFilters 와 따로 두는 이유가 있다. 저쪽은 '필터 초기화' 버튼을
// 띄울지 여부라 검색어까지 포함해 참/거짓만 보면 되고, 이쪽은 시트 안에 숨은
// 것이 몇 개인지 세야 한다. 값이 걸려 있는데 컨트롤이 시트 안에 들어가 있으면
// 사용자는 결과가 왜 좁아졌는지 알 수 없다 — 보이지 않는 필터가 제일 나쁘다.
//
// 검색어는 세지 않는다. 검색창은 시트 밖에 그대로 보인다.
//
...
export function countActiveFilters({ filters = {}, mine = false, includeDone = false } = {}) {
  let n = FILTER_KEYS.filter((key) => Boolean(filters[key])).length;
  if (mine) n += 1;
  if (includeDone) n += 1;
  return n;
}
