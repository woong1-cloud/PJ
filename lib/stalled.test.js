...
describe('확인 대기는 정체가 아니다', () => {
  it('물어보고 기다리는 중이면 멈춘 것으로 안 센다', () => {
    // 공이 요청자에게 있는데 '멈춘 것'으로 세면 우리가 안 집은 것처럼 보인다.
    // 실제로 검토대기 중앙값 17일 중 일부가 이 경우였다.
    expect(isStalled({ status: '검토대기', stalledDays: 30 })).toBe(true);
    expect(isStalled({ status: '검토대기', stalledDays: 30, awaitingAnswer: true })).toBe(false);
  });

  it('보류든 보통 상태든 마찬가지다', () => {
    expect(isStalled({ status: '보류', stalledDays: 90, awaitingAnswer: true })).toBe(false);
  });

  it('stalledDays 는 그대로 잰다 — 여기서 null 로 만들지 않는다', () => {
    // 답을 60일 안 주는 것도 언젠가 드러나야 하고, 화면은 '3일째 답 기다림'을
    // 그려야 한다.
    const days = stalledDays({
      requirement: { status: '검토대기', created_at: '2026-08-01T00:00:00Z' },
