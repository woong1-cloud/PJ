...
export const REQUIREMENT_STATUSES = [
  ...
  '승인대기',
  '완료',
  // 보류는 보드 상태 다음, 종결 셋 앞이다. 끝난 것이 아니라 미뤄둔 것이라
  // 반려·취소보다 앞에 있는 편이 목록에서 읽기 쉽다.
  '보류',
  '반려',
  '취소',
  '중복',
...
// 지금은 못 하지만 안 하기로 한 것은 아닌 상태.
//
// 반려로 적으면 요청자에게 거절로 읽히고, 취소로 적으면 요청자가 거둔 것이
// 된다. 운영 첫 3주의 반려 6건 중 셋이 실제로는 이것이었다.
export const HOLD_STATUS = '보류';
...
// IT가 그 건을 처음 손대는 상태. 착수 창(StartReviewDialog)이 뜨는 지점이다.
export const REVIEW_IN_PROGRESS_STATUS = '검토중';
...
// 아니라 "조용히 틀린 화면"이 된다.
//
// 보류가 여기 있는 것이 맞다. 목록에서 기본으로 숨겨야 "지금 손볼 것"이 흐려지지
// 않고, 회의 안건에서도 빠져야 한다 — 보류한 건을 다음 주에 다시 물으면 보류한
// 뜻이 없다. 다만 정체 판정만은 예외다(lib/stalled.js 의 stalledDays 참고):
// 보류는 끝난 것이 아니라 미뤄둔 것이라, 시간이 가는 것을 세야 잊히지 않는다.
