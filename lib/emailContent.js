...
//
// '의견'을 하나 더 늘렸다(2026-08). 인앱을 쓰지 않는 이유는 위와 같다 —
// 110건 중 105건이 안 읽혔다. 상한은 사람 수가 대신한다: 활성 19명에 실사용
// 4명이라 주 1~2건을 넘기 어렵다. 그보다 많아지면 묶어 보내는 쪽으로 다시
// 봐야 한다. 받는 사람도 전체 관리자 넷 이하다.
// '요건확인'(2026-09)도 의도적으로 늘렸다. 지금 메일이 나가는 사건 중 요청자
// 에게 가는 것은 회의배정 하나뿐인데, 요청자가 실제로 무언가 해야 하는 순간이
// 정확히 이것이다 — 답을 안 주면 그 건은 멈춘다. 상한은 건당 1통이고, 답이
// 오기 전에는 다시 안 나간다(재촉은 사람이 눌러야 한다).
...
// 메일 본문에 싣는 의견의 길이 상한. 넘으면 관리 화면에서 마저 읽는다.
const MAX_EMAIL_BODY = 600;
...
export function signupEmail({ name, organizationName, baseUrl }) {
  ...
  return {
    ...
    text: body({
      ...
      url: absoluteUrl(baseUrl, '/settings/members'),
    }),
  };
}
...
// 런칭 항목에서 불렸을 때. 요구사항용 mentionEmail 과 따로 두는 이유는
// 링크가 다르기 때문만이 아니다 — 항목 코드가 제목에 들어가야 메일함에서
// 무엇에 대한 이야기인지 읽힌다.
export function launchMentionEmail({ code, title, actorName, launchId, baseUrl }) {
  const by = actorName ? `${actorName}님이 ` : '누군가 ';
  return {
    subject: `${SUBJECT_PREFIX} 언급됨 — ${code} ${shorten(title)}`,
    text: body({
      lines: [`${by}런칭 항목 댓글에서 회원님을 언급했습니다.`, '', `· ${code} ${title}`],
      url: absoluteUrl(baseUrl, `/launch/${launchId}?task=${encodeURIComponent(code)}`),
    }),
  };
}

// 협조 요청 메일 — 고른 역할의 참여자에게.
//
// 제목에 항목 코드를 넣는다. launchMentionEmail 과 같은 이유다 — 런칭 항목
// 제목은 '상세페이지 오픈'처럼 짧고 여러 런칭에 같은 것이 있어서, 코드가
// 없으면 메일함에서 무엇에 대한 것인지 안 읽히고, 안 읽히면 안 열린다.
//
// 사람당 한 통이고, 항목이 여럿이면 목록을 본문에 담는다. 항목마다 보내면
// 3건 × 5명 = 15통이 되고, 그 순간 이 메일은 규칙으로 걸러진다 —
// assignedBatchEmail 이 건별 발송을 접은 것과 같은 판단이다.
export function helpRequestEmail({
  ...
  const by = actorName ? `${actorName}님이 ` : '누군가 ';
  const who = roles.filter(Boolean).join(' · ');
  const lines = [
    `${by}${who}에 협조를 요청했습니다.`,
    '',
    ...tasks.map((t) => `· ${t.code} ${t.title}`),
  ];

  // 한마디를 안 적고 보내는 것이 정상 경로다(역할만 고르면 충분할 때가 많다).
  // 비었을 때 빈 따옴표를 남기지 않는다 — 받는 사람은 앱이 값을 잃어버린
  // 것으로 읽는다. signupEmail 의 '빈 자리를 남기지 않는다'와 같다.
  const said = typeof message === 'string' ? message.trim() : '';
  if (said) lines.push('', `"${shorten(said, MAX_EMAIL_BODY)}"`);

  return {
    subject: one
      ? `${SUBJECT_PREFIX} 협조 요청 — ${one.code} ${shorten(one.title)}`
      : // 런칭 이름을 못 받았을 때 'undefined 3건'이 나가면 안 된다.
        `${SUBJECT_PREFIX} 협조 요청 — ${launchName ?? '런칭'} ${tasks.length}건`,
    text: body({
      lines,
      // 항목이 하나면 그 항목을 연다. 여럿이면 어느 하나를 고를 수 없으므로
      // 보드로 보낸다 — 거기서 고른 것들이 다 보인다.
      url: absoluteUrl(
        baseUrl,
...
// 새 의견 — 전체 관리자에게.
//
// 인앱 알림을 함께 보내지 않는다. 배치대기와 다른 점이다 — 저쪽은 벨을 보는
// 관리자가 바로 처리할 수 있는 일이지만, 의견은 읽고 판단하는 일이라 벨 한
// 줄로는 담기지 않는다. 그리고 지금 벨은 110건 중 105건이 안 읽혔다.
//
// 본문을 제목이 아니라 본문에 싣는다. 의견은 한두 문단이라 제목에 넣으면
// 잘리고, 잘린 문장은 열어 볼 이유를 못 만든다.
// 인자 이름이 body 가 아니라 message 인 것은 위의 body() 헬퍼와 겹치지 않게
// 하기 위해서다. 겹치면 이 함수 안에서 헬퍼를 부를 수 없다.
export function feedbackEmail({ name, message, baseUrl }) {
  const who = name ?? '누군가';
  return {
    subject: `${SUBJECT_PREFIX} 새 의견 — ${who}`,
    text: body({
      lines: [`${who}님이 의견을 남겼습니다.`, '', shorten(message, MAX_EMAIL_BODY)],
      url: absoluteUrl(baseUrl, '/settings/feedback'),
    }),
  };
}
...
// 요건 확인 — 요청자에게.
//
// 질문 전문을 본문에 싣는다. "확인이 필요합니다"만 보내면 열어 봐야 무엇을
// 묻는지 알 수 있고, 그 한 걸음에서 대부분이 멈춘다.
export function askEmail({ title, actorName, question, requirementId, baseUrl }) {
  const by = actorName ? `${actorName}님이 ` : '';
  return {
    subject: `${SUBJECT_PREFIX} 확인 요청 — ${shorten(title)}`,
    text: body({
      lines: [
        `${by}올리신 요청의 요건을 확인하고 싶어 합니다.`,
        '',
        `· ${title}`,
        '',
        shorten(question, MAX_EMAIL_BODY),
        '',
        '아래 링크에서 코멘트로 답해 주세요. 답이 오면 다음 회의에서 다시 봅니다.',
      ],
      url: absoluteUrl(baseUrl, `/requirements/${requirementId}`),
    }),
  };
