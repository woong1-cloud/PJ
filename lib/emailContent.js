...
//
// 제목에 안건 수를 적는다. 받는 사람이 제목만 보고 "이번 주는 회의에서 뭘
// 다루는지"를 알아야 열지 말지 정할 수 있다.
export function meetingDigestEmail({ brandName, meetingDate, digest, baseUrl }) {
  const where = brandName ? `[${brandName}] ` : '';
  const agenda = digest.stalled.count + digest.incoming.count;
  ...
  return {
    subject: `${SUBJECT_PREFIX} ${where}주간회의 안건 ${agenda}건`,
    text: body({ lines, url: absoluteUrl(baseUrl, '/meeting') }),
  };
}
