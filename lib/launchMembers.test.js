...
describe('memberRows', () => {
  ...
  it('사람은 이름순이다', () => {
    // 참관을 안 만들기로 해서 정렬 축이 이름 하나다.
    const rows = memberRows({
      tasks: [task({ owner_role: '재무팀' })],
      members: [mem('황경임', '재무팀'), mem('정소희', '재무팀')],
    });
    expect(rows[0].members.map((m) => m.member_id)).toEqual(['정소희', '황경임']);
  });
});

describe('assigneeId', () => {
  it('목록 API 가 주는 객체에서 id 를 꺼낸다', () => {
    expect(assigneeId(task({ assignee: { id: 'uuid-1', name: '한지웅' } }))).toBe('uuid-1');
  });

  it('문자열로 와도 그대로 준다 — 저장값과 PATCH 응답이 그 모양이다', () => {
    expect(assigneeId(task({ assignee: 'uuid-1' }))).toBe('uuid-1');
  });

  it('없으면 빈 문자열', () => {
    expect(assigneeId(task())).toBe('');
    expect(assigneeId(task({ assignee: null }))).toBe('');
    expect(assigneeId(undefined)).toBe('');
  });

  it('객체와 문자열을 그냥 === 로 견주면 안 된다는 것이 이 함수의 이유다', () => {
    const t = task({ assignee: { id: 'uuid-1', name: '한지웅' } });
    expect(t.assignee === 'uuid-1').toBe(false);
    expect(assigneeId(t) === 'uuid-1').toBe(true);
  });
});
...
describe('memberCandidates', () => {
  const people = [
    { id: 'b', name: '정소희', organization: { name: '재무' } },
    { id: 'a', name: '황경임', organization: { name: '재무' } },
    { id: 'c', name: '박천주', organization: { name: '스파오' } },
  ];
  const ids = (r) => r.hits.map((p) => p.id);

  it('이름순이다', () => {
    expect(ids(memberCandidates({ people }))).toEqual(['c', 'b', 'a']);
  });

  it('이미 그 역할로 들어온 사람은 뺀다', () => {
    const r = memberCandidates({ people, members: [mem('a', '재무팀')], roleName: '재무팀' });
    expect(ids(r)).toEqual(['c', 'b']);
    expect(r.total).toBe(2);
  });

  it('다른 역할로 들어온 사람은 남긴다 — 한 사람이 두 역할을 가질 수 있다', () => {
    const r = memberCandidates({ people, members: [mem('a', '법무팀')], roleName: '재무팀' });
    expect(ids(r).sort()).toEqual(['a', 'b', 'c']);
  });

  it('이름으로 찾는다', () => {
    expect(ids(memberCandidates({ people, query: '황' }))).toEqual(['a']);
  });

  it('소속으로도 찾는다 — 재무 조직 사람을 한 번에 고를 수 있어야 한다', () => {
    expect(ids(memberCandidates({ people, query: '재무' })).sort()).toEqual(['a', 'b']);
  });

  it('이미 고른 사람은 검색어와 안 맞아도 남긴다', () => {
    // 셋을 체크하고 다른 이름을 치는 순간 그 셋이 사라지면, 고른 것이
    // 없어진 줄 안다.
    expect(ids(memberCandidates({ people, query: '박', picked: ['a'] })).sort())
      .toEqual(['a', 'c']);
  });

  it('total 은 거르기 전 수다 — "19명 중 4명"을 쓸 수 있어야 한다', () => {
    const r = memberCandidates({ people, query: '황' });
...
describe('assigneeCandidates', () => {
  ...
  it('can_edit 이 거짓이면 뺀다 — 지금은 안 만들지만 보고 화면에서 되살아날 자리다', () => {
