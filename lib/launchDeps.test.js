...
describe('descendants', () => {
  it('건너건너 기다리는 것까지 모은다 — 자기 자신 포함', () => {
    // 01-01 → 01-02 → 01-03 이면 01-01 의 후손은 자기 + 둘이다.
    expect([...descendants({ code: '01-01', tasks: SAMPLE })].sort())
      .toEqual(['01-01', '01-02', '01-03', '01-18', '03-01']);
  });

  it('코드가 없으면 빈 집합 — 만들기 중인 항목', () => {
    expect([...descendants({ code: undefined, tasks: SAMPLE })]).toEqual([]);
  });

  it('고리가 있어도 안 돈다', () => {
    const a = t('A-01', { depends_on: ['B-01'] });
    const b = t('B-01', { depends_on: ['A-01'] });
    expect([...descendants({ code: 'A-01', tasks: [a, b] })].sort()).toEqual(['A-01', 'B-01']);
  });
});

describe('depCandidates', () => {
  it('자기 자신과 후손을 뺀다 — 고리를 원천에서 막는다', () => {
    // 07-33 이 자기를 선행으로 갖고 있어 07-33·07-34·07-35 셋이 영영
    // 착수 가능이 안 됐다. 그런 것을 고를 수 없게 한다.
    const { hits } = depCandidates({ code: '01-01', tasks: SAMPLE });
    expect(hits.map((x) => x.code)).toEqual([]);
  });

  it('나를 안 기다리는 것은 고를 수 있다', () => {
    const { hits } = depCandidates({ code: '03-01', tasks: SAMPLE });
    // 03-01 은 01-02 를 기다린다. 01-01·01-02 는 조상이라 고를 수 있고
    // 01-03·01-18 은 형제라 역시 고를 수 있다.
    expect(hits.map((x) => x.code)).toEqual(['01-01', '01-02', '01-03', '01-18']);
  });

  it('이미 고른 것은 뺀다', () => {
    const { hits } = depCandidates({ code: '03-01', tasks: SAMPLE, chosen: ['01-02'] });
    expect(hits.map((x) => x.code)).not.toContain('01-02');
  });

  it('해당없음은 뺀다 — 안 할 일을 기다릴 수 없다', () => {
    const tasks = [...SAMPLE, t('09-09', { status: '해당없음' })];
    const { hits } = depCandidates({ code: '03-01', tasks });
    expect(hits.map((x) => x.code)).not.toContain('09-09');
  });

  it('코드로도 제목으로도 찾힌다 — 손으로 치던 방식을 안 잃는다', () => {
    const tasks = [t('01-01', { title: '통신판매업 신고' }), t('02-01', { title: '상표권 이전' })];
    expect(depCandidates({ code: 'X-01', tasks, query: '01-01' }).hits.map((x) => x.code))
      .toEqual(['01-01']);
    expect(depCandidates({ code: 'X-01', tasks, query: '통신판매' }).hits.map((x) => x.code))
      .toEqual(['01-01']);
  });

  it('기한 이른 순이다', () => {
    const { hits } = depCandidates({ code: 'X-01', tasks: SAMPLE });
    expect(hits.map((x) => x.day_offset)).toEqual([-100, -95, -90, -75, -70]);
  });

  it('limit 만큼만 주고 total 로 몇 건인지 알린다', () => {
    const r = depCandidates({ code: 'X-01', tasks: SAMPLE, limit: 2 });
