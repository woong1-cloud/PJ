...
import { firstLookRows, restRows, templateRow, statusRow, optionsFrom } from './launchTemplate';
...
describe('statusRow', () => {
  it('양식의 열 이름과 겹치는 부분이 COL 과 여전히 일치한다', () => {
    const row = statusRow({ task: task(), openDate: OPEN, decisionsById: new Map() });
    const parserKeys = Object.values(COL).filter((name) => name !== COL.progress);
    for (const key of parserKeys) {
      expect(Object.prototype.hasOwnProperty.call(row, key)).toBe(true);
    }
  ...
  it('양식 열 + 진행 열이 전부 채워진다', () => {
    const decisionsById = new Map([['d1', { id: 'd1', title: '재고 운영 방식' }]]);
    const row = statusRow({
      task: task({
        status: '막힘',
        assignee_name: '홍길동',
        done_at: '2026-09-01T10:23:45.123Z',
        blocked_reason: '재고 운영 방식 미결',
        blocked_decision_id: 'd1',
        excluded_reason: null,
        source: 'import',
        plain_text: '새 회사를 세우고 물건을 팔 허가를 받는 일',
      }),
      openDate: OPEN,
      decisionsById,
    });
    expect(row['상태']).toBe('막힘');
    expect(row['담당자']).toBe('홍길동');
    expect(row['완료일']).toBe('2026-09-01');
    expect(row['막힌 이유']).toBe('재고 운영 방식 미결');
    expect(row['기다리는 결정']).toBe('재고 운영 방식');
    expect(row['해당없음 사유']).toBe('');
    expect(row['출처']).toBe('엑셀');
    expect(row['쉬운 설명']).toBe('새 회사를 세우고 물건을 팔 허가를 받는 일');
  });

  it('완료일은 시각을 버리고 날짜만 남는다', () => {
    const row = statusRow({ task: task({ done_at: '2026-09-01T10:23:45.123Z' }), openDate: OPEN });
    expect(row['완료일']).toBe('2026-09-01');
  });

  it('done_at 이 없으면 완료일은 빈 문자열', () => {
    const row = statusRow({ task: task({ done_at: null }), openDate: OPEN });
    expect(row['완료일']).toBe('');
  });

  it('blocked_decision_id 가 없으면 기다리는 결정은 빈 칸', () => {
    const row = statusRow({ task: task({ blocked_decision_id: null }), openDate: OPEN, decisionsById: new Map() });
    expect(row['기다리는 결정']).toBe('');
  });

  it('blocked_decision_id 가 decisionsById 에 없으면 기다리는 결정은 빈 칸', () => {
    const row = statusRow({
      task: task({ blocked_decision_id: 'missing' }),
      openDate: OPEN,
      decisionsById: new Map([['d1', { id: 'd1', title: '다른 결정' }]]),
    });
    expect(row['기다리는 결정']).toBe('');
  });

  it('decisionsById 가 없어도 안 죽는다', () => {
    const row = statusRow({ task: task({ blocked_decision_id: 'd1' }), openDate: OPEN });
    expect(row['기다리는 결정']).toBe('');
  });

  it("source 가 manual 이면 출처는 손으로 넣음, 아니면 엑셀", () => {
    const manual = statusRow({ task: task({ source: 'manual' }), openDate: OPEN });
    const guide = statusRow({ task: task({ source: 'guide' }), openDate: OPEN });
    const noSource = statusRow({ task: task({ source: undefined }), openDate: OPEN });
    expect(manual['출처']).toBe('손으로 넣음');
    expect(guide['출처']).toBe('엑셀');
    expect(noSource['출처']).toBe('엑셀');
  });

  it('비고는 원본 그대로다 — 해당없음이어도 excluded_reason 으로 바뀌지 않는다', () => {
    const row = statusRow({
      task: task({ status: '해당없음', note: '원래 비고', excluded_reason: '이 브랜드는 자사몰 없음' }),
      openDate: OPEN,
    });
    expect(row['비고']).toBe('원래 비고');
    expect(row['해당없음 사유']).toBe('이 브랜드는 자사몰 없음');
  });

  it('빈 입력에서 안 죽는다', () => {
    expect(statusRow({})).toBe(null);
    expect(statusRow({ task: null, openDate: OPEN })).toBe(null);
    expect(statusRow()).toBe(null);
  });
});
