...
// 참여자가 앞, 참관이 뒤. 그다음 이름순.
//
// 참관을 뒤로 보내는 이유: 이 화면에서 찾는 것은 "누가 이 역할의 일을
// 하나"이고, 참관은 그 답이 아니다.
function sortMembers(list) {
  return [...list].sort(
    (a, b) =>
      Number(b.can_edit !== false) - Number(a.can_edit !== false) ||
...
export function memberCandidates({ people = [], members = [], roleName = '' } = {}) {
  const taken = new Set(
