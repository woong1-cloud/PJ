...
// 문자열과 === 로 견주면 늘 거짓이고, 1단계에서 「내 담당」이 실제로 그
// 버그로 영영 0이었다.
...
// 종결 사유를 벨 문구에 이어 붙일 때의 길이 상한.
//
// 벨은 한 줄이라 길면 잘린다. 다 담으려 하면 앞의 "무엇이 바뀌었나"가 밀려
// 나가므로, 사유는 첫머리만 보여 주고 나머지는 눌러서 읽게 한다.
const MAX_REASON = 40;

// status: 바뀐 상태. reason: 반려·취소 사유 또는 승인 확인 내용(선택).
//
// 사유를 붙이는 이유: 반려 알림이 "상태를 '반려'로 바꿨습니다"뿐이면 받는
// 사람은 왜인지 알 수 없고, 들어와서 찾아야 한다. 실제로 사유는 저장되는데
// 화면 어디에서도 안 보인다는 말이 나왔다.
export function statusMessage(actorName, title, status, reason) {
  const head = `${actorPhrase(actorName)} '${truncateTitle(title)}' 상태를 '${status}'${roParticle(
    status
  )} 바꿨습니다`;
  const trimmed = typeof reason === 'string' ? reason.trim() : '';
  if (!trimmed) return head;
  const short =
    trimmed.length <= MAX_REASON ? trimmed : `${trimmed.slice(0, MAX_REASON)}…`;
  return `${head} — ${short}`;
}
...
// 접수(검토 요청) 알림의 수신자.
//
// 이 알림만 규칙이 다르다. 나머지는 "당신 건에 무슨 일이 있었다"라서 요청자·
// 담당자에게만 가지만, 접수는 "아직 아무도 안 잡은 일이 생겼다"이다. 제출
// 시점에는 담당자가 정의상 없으므로 resolveRecipients 를 쓰면 요청자 본인만
// 후보에 오르고, 그 사람이 곧 행위자라 수신자가 0명이 된다.
//
// 실제로 그랬다. 34건이 등록되는 동안 접수 알림이 한 건도 나가지 않았고
// (인앱 알림 기록에 가입·담당자지정·멘션만 있었다), 34건 중 30건이 담당자
// 없이 남았다. 올린 사람은 접수됐다고 믿고 IT 는 목록을 직접 열기 전까지
// 몰랐다.
//
// 그래서 범위를 "그 브랜드에서 이 일을 잡을 수 있는 사람"으로 잡는다 —
// 3차 이상. 등급으로 뿌리는 것이 위험하다는 건 이 파일 위쪽에 적힌 그대로지만
// (벨이 장식이 된다), 접수는 하루 한 건 수준이고 받는 사람은 브랜드당 몇 명이다.
// 무엇보다 이 알림은 받은 사람끼리 "누가 잡을지"를 정하라고 보내는 것이다.
//
// roles: [{ team_member_id, tier }] — user_brand_roles 를 그 브랜드로 좁혀 넘긴다.
export function submitRecipients(roles = [], actorId = null) {
  const recipients = [];
  for (const role of Array.isArray(roles) ? roles : []) {
    const id = role?.team_member_id;
    // 행위자 제외는 다른 알림과 같은 규칙이다. 3차 이상이 자기 건을 직접
    // 올리는 경우가 있는데(실무자도 요청자가 될 수 있다) 그때 자기 벨이
    // 울리면 안 된다.
    if (!id || id === actorId || recipients.includes(id)) continue;
    if ((TIER_RANK[role?.tier] ?? 0) < TIER_RANK['3차']) continue;
    recipients.push(id);
  }
  return recipients;
}
...
// 접수 알림 문구.
//
// statusMessage 를 쓰지 않는다. 그건 "'검토대기'로 바꿨습니다"가 되는데,
// 받는 사람 입장에서 할 일이 무엇인지가 없다. 가입 알림과 같은 이유로
// 문장이 "무엇을 해야 하는가"로 끝난다.
export function submitMessage(actorName, title) {
  return `${actorPhrase(actorName)} '${truncateTitle(title)}' 검토를 요청했습니다 — 담당자 지정이 필요합니다`;
}
...
// 배치를 하러 가야 한다.
//
// organizationName: 가입자가 고른 조직 이름('스파오', '법무팀' …).
// 어느 조직 사람이 기다리는지가 배치 판단의 절반이다.
//
// 예전에는 소속('브랜드'/'본부')과 근무 브랜드를 따로 받아 '스파오(브랜드)'
// 처럼 조합했다. 소속이 실제 조직명이 되면서 두 값이 하나로 합쳐졌고, 그대로
// 뒀다면 '법무팀(소속) 소속으로' 같은 문장이 나갔을 것이다.
export function signupMessage(name, organizationName) {
  const who = name ? `${name}님이` : '새 사용자가';
  const scope = organizationName ? ` ${organizationName} 소속으로` : '';
