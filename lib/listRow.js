...
// awaiting: lib/awaitingAnswer.js 의 { since, days } 또는 null
//
// 반환: { tone, flag, channelBadge, elapsed, meta, tail, assignee, awaiting }
export function listRow({
  ...
  const stalled = isStalled({ status, stalledDays, awaitingAnswer: Boolean(awaiting) });
  ...
  return {
    ...
    // 확인 대기면 경과 대신 그것을 말한다. '17일째'는 우리가 안 집었다는
    // 뜻으로 읽히는데, 실제로는 답을 기다리는 중이다.
    elapsed: awaiting
      ? awaiting.days === null
        ? '답 기다리는 중'
        : `${awaiting.days}일째 답 기다림`
      : resolveElapsed({ requirement, status, closed, hold, stalled, stalledDays }),
    meta: [
    ...
    // 목록에서는 감추지 않고 표시만 한다. 요청자가 자기 건을 볼 때 "내 답을
    // 기다리는 중"이 보여야 한다 — 감추면 물어본 사실 자체가 안 보인다.
    awaiting: awaiting ? { days: awaiting.days } : null,
  };
}
