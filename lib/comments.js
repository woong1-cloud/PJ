...
// 코멘트에 붙은 시안까지 함께 읽는 select.
//
// 여기서도 FK 이름을 못 박는다. requirement_images 는 requirement_id 로도
// 이 건에 매달려 있어서, 짧게 쓰면 PostgREST 가 어느 관계로 들어갈지 못 고른다.
//
// signedUrl 은 여기서 못 만든다. storage_path 를 그대로 꺼내고, 서명은
// 라우트가 signCommentImages 로 한 번에 한다(lib/storage.js).
export const COMMENT_SELECT_WITH_IMAGES =
  `${COMMENT_SELECT}, ` +
  'images:requirement_images!requirement_images_comment_id_fkey' +
  '(id, storage_path, content_type, file_name)';

// 런칭 항목 코멘트용. requirement 쪽과 컬럼도 FK 이름도 달라 따로 둔다.
//
// 여기서도 FK 이름을 못 박는다. 안 박으면 PGRST201(모호한 관계)이 난다 —
// launch_task_comments → team_members 로 가는 길이 author 하나뿐이어도,
// 이름을 적어 두면 나중에 컬럼이 늘어도 안 깨진다.
export const LAUNCH_COMMENT_SELECT =
  'id, task_id, body, created_at, edited_at, ' +
  'author:team_members!launch_task_comments_author_fkey(id, name)';
