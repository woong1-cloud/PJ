...
// 코멘트 시안은 훨씬 적게 잡는다. 코멘트는 한 마디이고 시안은 그 한 마디에
// 붙는 그림이라 보통 한두 장이다. 요구사항 첨부와 같은 15개를 열어 두면
// 대화 한 칸이 화면을 통째로 먹는다 — 여러 장이 필요하면 요구사항 첨부다.
...
// 첨부 1개 기준 판정(순수 함수).
//
// max 는 부르는 쪽이 정한다. 요구사항 첨부와 코멘트 시안이 한 테이블을 쓰지만
// 세는 범위가 다르다 — 요구사항은 건 전체, 코멘트는 그 코멘트 안이다.
export function validateAttachmentUpload({
  contentType,
  byteSize,
  currentCount,
  max = MAX_ATTACHMENTS_PER_REQ,
}) {
  ...
  if (currentCount >= max) {
    return { ok: false, error: `첨부는 최대 ${max}개까지 가능합니다.` };
  }
  return { ok: true };
}
