...
// 코멘트 목록에 붙은 시안들의 서명 URL 을 한 번에 만든다.
//
// 코멘트마다 toSignedImageList 를 부르면 코멘트 수만큼 서명 요청이 나간다.
// 스무 개짜리 대화에서 스무 번이다. 경로를 전부 모아 한 번에 서명한다.
export async function signCommentImages(comments) {
  const rows = (comments ?? []).flatMap((c) => c.images ?? []);
  const signed = await signImagePaths(rows.map((r) => r.storage_path));
  return (comments ?? []).map((c) => ({
    ...c,
    images: (c.images ?? []).map((r) => ({
      id: r.id,
      signedUrl: signed[r.storage_path] ?? null,
      content_type: r.content_type,
      file_name: r.file_name ?? null,
