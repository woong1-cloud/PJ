...
// 라우트(help-requests/recent)도 같은 창으로 댓글을 긁는다. 내보내지 않으면
// 값이 두 곳에 적히고 한쪽만 고쳤을 때 조용히 갈라진다.
...
//
// excludeId 는 보내는 사람이다. 자기가 보낸 메일을 자기가 받으면 그때부터
// 이 메일은 소음이다. **세는 곳과 보내는 곳이 같은 목록을 봐야 한다** —
// 라우트가 여기서 안 빼고 notify 만 빼면 화면이 「5명에게 보냈습니다」라
// 말하고 4명이 받는다.
export function recipientsForRoles(members = [], roles = [], excludeId = null) {
  const wanted = new Set((roles ?? []).filter(Boolean));
  if (wanted.size === 0) return [];
  const list = mentionableFromMembers(
