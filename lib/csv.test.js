...
describe('requirementsToCsv', () => {
  ...
  it('헤더 순서대로 값을 채운다', () => {
    ...
    expect(lines[1]).toBe(
      '오류,개발중,자사몰,"장바구니, 쿠폰 오류",주문,상,김지원,박실무,2026-08-01,2026-08-20,,결제 개편,https://r/issues/1234,2,쿠폰이 두 번 적용된다,한 번만 적용,급함'
    );
  });
  ...
  it('본문은 맨 뒤에 온다 — 왼쪽은 훑는 영역, 오른쪽은 들여다보는 영역', () => {
    expect(REQUIREMENT_CSV_HEADERS.slice(-3)).toEqual(['As-Is', 'To-Be', '비고']);
  });

  it('detail=true 로 받지 않아 본문이 없어도 표 모양이 유지된다', () => {
    // 목록 화면은 detail 없이 부르므로 as_is/to_be/note 가 아예 없다.
    // 그때도 헤더 수만큼 칸이 나와야 엑셀에서 열이 밀리지 않는다.
    const withoutBody = { ...req };
    delete withoutBody.as_is;
    delete withoutBody.to_be;
    delete withoutBody.note;
    expect(requirementsToCsv([withoutBody]).endsWith(',,,')).toBe(true);
  });
