...
describe('suggestTierFromOrg', () => {
  ...
  // 1차는 브랜드별 등급이 아니다. 저장할 수 없는 값을 제안하면 배치 창이
  // 그것으로 채우고 서버가 400 을 내서, 그 소속인 사람을 아예 배치할 수
  // 없게 된다. 「사업부문」의 기본등급이 실제로 1차였다.
  it('1차는 제안하지 않는다 — 저장할 수 없는 값이다', () => {
    expect(suggestTierFromOrg({ default_tier: '1차' })).toBe('4차');
  });
