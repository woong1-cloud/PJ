...
describe('computeCompletedAt', () => {
  it('완료로 진입하면 nowIso로 설정한다', () => {
    expect(computeCompletedAt('개발중', '완료', null, NOW)).toBe(NOW);
  });

  it('완료에서 벗어나면 null로 초기화한다', () => {
    expect(computeCompletedAt('완료', '개발중', '2026-01-01T00:00:00.000Z', NOW)).toBe(null);
  });
  ...
  it('완료와 무관한 전이는 기존 값(null)을 유지한다', () => {
    expect(computeCompletedAt('작성중', '개발중', null, NOW)).toBe(null);
  });
});
