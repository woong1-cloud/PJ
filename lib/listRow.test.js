...
describe('listRow — 보류', () => {
  const hold = (over = {}) => req({ status: '보류', ...over });

  it('60일 전까지는 깃발이 없다', () => {
    // 보통 상태라면 40일은 한참 전에 붉어졌다. 보류는 미뤄 두기로 한 것이라
    // 그 기간에 재촉하지 않는다.
    expect(listRow({ requirement: hold(), stalledDays: 40 }).flag).toBe(null);
  });

  it('60일을 넘으면 붉게 드러난다', () => {
    const row = listRow({ requirement: hold(), stalledDays: 62 });
    expect(row.flag).toBe('stall');
    expect(row.elapsed).toBe('62일째 멈춤');
  });

  it('59일과 60일 사이가 경계다', () => {
    expect(listRow({ requirement: hold(), stalledDays: 59 }).flag).toBe(null);
    expect(listRow({ requirement: hold(), stalledDays: 60 }).flag).toBe('stall');
  });

  it('보류 사유가 있으면 함께 보인다', () => {
    // 무엇이 풀려야 다시 움직이는지가 사유에만 적힌다. 목록에서 안 보이면
    // 건마다 열어 봐야 한다.
    const row = listRow({
      requirement: hold(),
      stalledDays: 10,
      closure: { status: '보류', label: '보류', reason: 'API 개편 후', by: '김담당', at: null },
    });
    expect(JSON.stringify(row)).toContain('API 개편 후');
  });
});
