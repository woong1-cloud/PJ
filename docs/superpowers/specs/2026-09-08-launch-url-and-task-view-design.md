...
```jsx
<Link href={`/launch/${launch.id}?view=blocked&role=`}>보드에서 열기 ↗</Link>
<Link href={`/launch/${launch.id}?view=late&role=`}>보드에서 열기 ↗</Link>
<Link href={`/launch/${launch.id}?view=week&role=`}>보드에서 열기 ↗</Link>
```

**`role=` 를 빈 값으로 붙이는 것이 요점이다.** 이 화면의 숫자는 런칭 전체
기준인데 그냥 보내면 보드가 브라우저에 기억된 역할을 얹는다 — 실제로
「기한 지남 2」를 눌렀더니 0건인 화면이 떴다(기억이 재무팀이었고 재무팀에는
지난 항목이 없다). 숫자가 데려간 곳에 그 숫자가 없으면 안 된다.
주소의 role 은 localStorage 를 안 덮어쓰므로 그 사람의 기본 역할은 남는다.

**0건인 칸에는 링크를 안 그린다.** 갈 곳이 빈 목록인 링크는 실망만 준다.

**④ 이번 주 마감에도 단다.** `view=week` 에 정확히 대응한다 — 처음에는
빼려 했는데 뺄 이유를 댈 수 없었다.

**①결정 대기·⑤이번 주 완료에는 안 단다.** ①은 항목이 아니라 결정이라 보드에
대응하는 보기가 없고, ⑤는 보드의 보기 여섯에 '완료'가 없어서 가리킬 주소가 없다.
