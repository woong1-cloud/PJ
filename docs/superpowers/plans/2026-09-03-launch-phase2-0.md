...
계획을 그대로 따르지 않은 자리가 셋이다. 남겨 둔다 — 다음 계획을 쓸 때
같은 실수를 안 하려면 무엇이 틀렸는지가 보여야 한다.

**① `planReimport` 에 `markNa` · `busy` 를 더했다** (커밋 `c61b266`)

Task 8 을 만든 에이전트가 찾았다. 이미 있는 항목이 나중 시트에서 해당없음으로
표시되면 `update` 경로로 가는데, 계획 열만 덮으므로 그 표시가 무시됐다.
**브랜드가 양식을 채우는 흐름이 바로 이것이라, 빠지면 양식을 만든 의미가
없다.**

스펙 안에 모순이 있었다 — 2.5절은 "상태는 안 건드린다", 2.3절은 "해당없음은
진행 상태가 아니라 범위라서 읽는다". 후자가 맞다.

다만 `할 것` 인 것만 바꾼다. `하는 중`·`막힘`·`완료` 는 `busy` 로 알리기만
한다 — 사람이 이미 붙어 있는 일을 파일이 지우면 안 된다.

**② Task 10 과 12 를 묶었다**

둘 다 `app/api/launch/[id]/route.js` 를 건드린다. 나눠 던지면 서로 덮어쓴다.

**③ Task 13 을 13a / 13b 로 쪼갰다**

13a 까지만 되어도 배포할 값이 있다 — 시트의 제외 25건이 해당없음으로 들어오고,
분모가 451이 되고, 전제 7줄이 뜬다. 13b(다중 선택)는 그 다음이다.

### 병렬 실행에서 배운 것

여러 에이전트를 한 작업 디렉터리에 띄웠더니 **git 인덱스가 공유**라 사고가 났다.
A 가 `git add` 한 직후 B 가 `git commit` 하면 A 의 파일까지 쓸려 들어간다.
더 나쁜 것은 그것을 발견한 에이전트가 `git reset --soft` 로 되돌리다가 그 사이
얹힌 **남의 커밋을 날린 것**이다. 커밋 둘이 실제로 브랜치에서 떨어졌고
`reflog` 로 복구했다.

다음부터:
- 커밋은 `git commit -m "…" -- <경로>` 로. `git add` 를 쓰지 않는다
  (새 파일은 `git add -- <그 경로 하나>` 뒤에 같은 경로로 커밋)
- 에이전트에게 `git reset`·`--amend`·`rebase` 를 **금지**한다. 남의 파일이
  섞여도 고치려 하지 말고 보고만 하게 한다 — 되돌리려는 시도가 사고였다
- 같은 파일을 만지는 과제는 반드시 순서대로
